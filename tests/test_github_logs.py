...
class TestFetchFailedJobLogs(unittest.TestCase):
    ...
    def test_sanitizes_job_names_and_logs(self):
        checks = [
            {
                "name": "job\u00a0one\x1b",
                "state": "FAILURE",
        ...
        self.assertEqual(str(out[0][0]), "job one\\x1b")
        self.assertEqual(str(out[0][1]), "FAILED:\\x00 nope\nnext")


class TestLatestDrciSummary(unittest.TestCase):
    _HEADER = (
        "As of commit aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa "
        "with merge base bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
    )
    _TRAILER = (
        "This comment was automatically generated by Dr. CI and "
        "updates every 15 minutes."
    )

    def test_picks_latest_pytorch_bot_with_trailer(self):
        comments = [
            {"author": "pytorch-bot", "body": f"{self._HEADER}\nold\n\n{self._TRAILER}"},
    ...
    def test_drops_summary_when_sha_does_not_match(self):
        ...
        body = (
            ...
            "As of commit aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa "
            "with merge base bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb (abbrev)\n"
    ...
    def test_drops_body_without_structural_header(self):
        # A pytorch-bot comment that carries the trailer but lacks dr. ci's
        # "As of commit ... with merge base ..." header is not trusted as a
        # CI summary -- even with no head_sha to cross-check. This is the
        # echoed-bot-reply spoofing vector.
        body = (
    ...
    def test_drops_header_not_at_line_start(self):
        # A header-shaped substring embedded mid-line (as an echoed reply
        # could contain) must not satisfy the anchored match.
        body = (
            "user said: As of commit aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa "
            "with merge base bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb do X\n"
            "\nThis comment was automatically generated by Dr. CI and updates every 15 minutes."
        )
        comments = [{"author": "pytorch-bot", "body": body}]
        self.assertIsNone(
            latest_drci_summary(
                comments, head_sha="aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
            )
        )

    def test_sanitizes_summary_text(self):
        body = (
            f"{self._HEADER}\n"
            "summary\x00with\u00a0spaces\u2028next\n"
            "\nThis comment was automatically generated by Dr. CI and updates every 15 minutes."
        )
        comments = [{"author": "pytorch-bot", "body": body}]

        self.assertIn(
            "summary\\x00with spaces\nnext",
            latest_drci_summary(comments) or "",
        )
